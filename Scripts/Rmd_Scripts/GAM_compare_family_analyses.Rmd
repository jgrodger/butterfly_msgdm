...
plot(gam_object, residuals = TRUE, seWithMean = TRUE, shade = TRUE, 
...
gam_ziP <- update(gam_poisson, family = ziP) #ziP for zero inflated Poisson data, when the zero inflation rate depends simply on the Poisson mean.
...
```{r}
gam_fam_list <-list (gam_gaussian, gam_poisson, gam_nb, gam_tweedie, gam_ziP)
