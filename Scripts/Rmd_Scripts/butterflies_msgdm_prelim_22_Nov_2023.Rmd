...
write_rds(zeta.ddecays, file = here("./Output/zeta_decay_orders_100_sam_10000.rds"))
```
...
```{r}
perf1 <- get_k_fold_performance(
  data.spec = bird.spec.fine[ ,3:192],
  data.env = bird.env.fine[,3:9],
  xy = bird.env.fine[,1:2],
  order = 3,
  sam = 100,
  reg.type = "ispline",
  normalize = "Simpson",
...
#check that different seeds give different output
perf2 <- get_k_fold_performance(
  data.spec = bird.spec.fine[ ,3:192],
  data.env = bird.env.fine[,3:9],
  xy = bird.env.fine[,1:2],
  order = 3,
...
# this is a check that it is reproducible for same seeds
perf2_copy <- get_k_fold_performance(
  data.spec = bird.spec.fine[ ,3:192],
  data.env = bird.env.fine[,3:9],
  xy = bird.env.fine[,1:2],
  order = 3,
...
perf_bird_zeta3 <- get_k_fold_performance(
  data.spec = bird.spec.fine[ ,3:192],
  data.env = bird.env.fine[,3:9],
  xy = bird.env.fine[,1:2],
  order = 3,
...
perf_bird_zeta5 <- get_k_fold_performance(
  data.spec = bird.spec.fine[ ,3:192],
  data.env = bird.env.fine[,3:9],
  xy = bird.env.fine[,1:2],
  order = 5,
...
perf_butterfly_zeta3 <- get_k_fold_performance(
  data.spec = site.by.species,
  data.env = site.by.env,
  xy = site.by.xy,
  order = 3,
  sam = 50000,
  reg.type = "ispline",
...
# Running get_k_fold_performance() generates warnings that are not an issue, 
# the second warning means that truncating step size is necessary in an algorithm to get the model to converge
#Zeta.msdgm gives two warnings: 
#Warning: non-integer #successes in a binomial glm! Warning: step size truncated due to divergence
#Predict.msgdm gives this warning
...
perf_butterfly_zeta3_sam_250000 <- get_k_fold_performance(
  data.spec = site.by.species,
  data.env = site.by.env,
  xy = site.by.xy,
  order = 3,
...
perf_butterfly_zeta3_sam_250000

write_csv(perf_butterfly_zeta3_sam_250000, file = here("./Output/butterfly_k_fold_250000_zeta3.csv"))
...
```{r}
z3  <- Zeta.order.mc.sam(data.spec = site.by.species, normalize = "Simpson", order=3, sam=500)

check <- data.frame(z3$zeta.val.vec)
...
tic(" validation script sams = 50000, zeta order = 3")
comp1 <- get_validation_different_seeds_and_samps(  
  data.spec = site.by.species,
  data.env = site.by.env,
  xy = site.by.xy,
  order = 3,
...
tic("msgdm sam = 50000")
set.seed(2)
msgdm.3a <-  Zeta.msgdm(xy = site.by.xy, data.spec = site.by.species, data.env = site.by.env,  order=3, sam=50000,
  reg.type="ispline", normalize="Simpson", family=binomial(link="log"),cons.inter = -1, glm.init = TRUE)
toc()
