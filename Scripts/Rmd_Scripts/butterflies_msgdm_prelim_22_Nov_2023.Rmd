...
perf_butterfly_zeta3_sam_500 <- get_k_fold_performance(
  data.spec = site.by.species,
  data.env = site.by.env,
  xy = site.by.xy,
  order = 3,
...
perf_butterfly_zeta3_sam_500

toc()
```

```{r}
tic("msgdm order = 3 sam = 500")
set.seed(1)
msgdm.3.500 <-  Zeta.msgdm(xy = site.by.xy, data.spec = site.by.species, data.env = site.by.env,  order=3, sam=500,
  reg.type="ispline", normalize= "Simpson", family=binomial(link="log"), cons.inter = -1, glm.init = TRUE)
toc()

summary(msgdm.3.500$model)
