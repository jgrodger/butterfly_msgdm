...
#this file from Rob Cooke includes type of survey for each site
...
#basemap withpit outlying islands
...
## combine records for small and essex skipper (species.nos 119 and 120), including those where observer recorded it as either of the two (species.no 200) 
```{r}
...
nrow(species)
```
## recode records for the two skipper species in the records dataframe to 1000 to match species dataframe
```{r}
...
print("Rows of records after combining skippers ") #yes,supposed to be the same
...
## filter out non-butterflies and low taxonomic resolution and no butterflie seen from species dataframe

```{r}
species <- species %>%
  filter(include ==1)

print("rows of species after removing moths and low taxonomic resolution and no butterfles seen categories")
nrow(species)

```
## filter out non-butterflies and low taxonomic resolution and no butterflie seen from records dataframe and remove any sites that no longer appear in records from sites

```{r}



#remove records of  moths and low taxonomic resolution and no butterfles seen categories
records <- records %>%
...
## remove records from sites no longer in the sites dataframe and remove species from species dataframe that are no longer in records
```{r}
...
aggregated.records <-  records.keep %>%
...
#I want this as a character, not just a number
...
# data to merge into sites dataframe
cov.merge <- coverage %>%
...
#filter sites dataframeto thos with > 99% coverage
...
# get some descriptive statistics for sites to include
sum3 <- sites.15.19 %>%
