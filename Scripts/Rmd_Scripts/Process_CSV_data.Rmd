...
This script loads ukbms data for records, sites and species. It cleans data, 
renames columns for ease of use, filters by various criteria, including coverage 
and sampling effort, and saves data for further use (combining with env vars 
and analyses)
...
# function for filtering data: assumes that the environment contains objects 
# called records, sites, species as prepared above. The argument is the one that
# dataset that will already have been filtered by another criterion. The other 
...
filter_ukbms <- function(filter_by){
  ...
  sites <-  filter(sites, site %in% unique(records$site))  
...
# Filter records  remaining species
...
##Some sites have incomplete info, one site remains with latitude and longitude in wrong columns and no other info, such sites are filtered on other criteria
...
# and the number of well surveyed years for each (sampled in all 4 periods)
...
#extract survey data to merge intor records
...
#### Find sites with 8 surveys in at least two years also satifying criteria above
```{r}
...
# Replace number of surveys with 1 if >= 8 or 0 otherwise
enough <- surveys.by.year[, 2:6] 
enough[enough < 8] <- 0
enough[enough >= 8] <- 1
# A column indicating if at least two years have enough surveys
...
```{r}
#### Sum records for each species in each site across years 
records <-  records %>%
...
#### redo site.by.species matrices after filtering
```{r}
