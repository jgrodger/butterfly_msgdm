...
#this may be redundant but it is done to remove  Irish sites
sites <- sites %>%
...
sites <- sites%>%
  filter(even == T,
         country != "Channel Isles" & country != "Isle of Man",
               site != 1791 & site != 1837) %>% #filter 138 Irish sites
...
species <- species %>% 
  filter (species.no %in% unique(records$species.no)) #species after filtering out Ireland,  Channel Islands, Lundy, Scilly, Isle of Man"
...
## filter out species non-butterflies and "unidentified" from species
```{r}
species <- species %>% 
...
print("records after removing non-butterflies and unidentified")
nrow(records)
...
species <- species %>% 
  filter (strategy == "Wider countryside sp"| strategy == "Habitat specialist") #migrants
...
print("sites after removing on-butterflies and unidentified")
nrow(sites)

print("species after removing on-butterflies and unidentified")
nrow(species)
...
sum2
# I thought it odd that some sites had more surveys than there are weeks in a year
#inspecting shows that these sites were surveyed multiple times per week in these years
check<- records %>%
...
site.and.year <- dplyr::select(surveys.site.year, site, year)
...
#yes, I know this is a copy and paste of the lines above
...
print("number of sites in 2015-2019 with at least 5 months data in at least three years")
nrow(sum.5.year )

#site year combinations to keep (site-years surveyed in at least 5 months, sites with at least three years such data)  
site.years.keep <- sum.months %>%
  filter(year %in% 2015:2019,
         months.surveyed >=5,
         site %in% sites.keep$site) 

print("check that I have same number of sites in site.year.keep and in")
length(unique(site.years.keep$site))

  
print("number of sites in 2015-2019 with years surveyed in at least 5 months, sites with at least three years such data")
length(unique(site.years.keep$site) )
...
```
# for 2015-2019 records combine T. sylvestris and T. lineola, then get site by species presence absence mat
```{r}
...
check<- filter(site.by.obs.5.year.counts, rowSums(site.by.obs.5.year.counts) == 0)

check1<- filter(records, site == 3325, year %in% 2015:2019)
...
sites.15.19 <- sites %>%
  filter(site %in% sites.keep$site)
...
check<- filter(sites.15.19, coverage > 0.995)
