...
#### Deviance explained pie charts (also rough)
```{r}
varparts <- list()

for (i in 1:length(params$orders_msgdm)) {
  set.seed(1)
  varparts[[i]] <- Zeta.varpart(msgdm.mod = my_msgdms[[i]], method.glm = "glm.fit2", cons.inter = -1) 
}
names(varparts) <- paste0("zeta.varpart.", params$orders_msgdm)

par(mfrow = c(1, length(params$orders_msgdm)))
for (i in 1:length(params$orders_msgdm)) {
   print(pie.neg(varparts[[i]][4:7,1], density = c(4, 0, 8, -1),  angle = c(90, 0, 0, 0), 
          labels =  c("distance","undistinguishable","environment","unexplained"), 
          radius = 0.9, main = paste0("Zeta = ", params$orders_msgdm[i])))
}


filepath <- paste0(out_path, "/msgdm_var_pie",".png")

#For unknown reasons, over writing is not happening  
unlink(filepath, recursive = FALSE, force = FALSE) #deletes previous version with the same name  
png(file = filepath,
   width = 130,
   height = 70*length(forest_plots),
   units = "mm",
   res = 72)
par(mfrow = c(length(params$orders_msgdm), 1))
for (i in 1:length(params$orders_msgdm)) {
   print(pie.neg(varparts[[i]][4:7,1], density = c(4, 0, 8, -1),  angle = c(90, 0, 0, 0), 
          labels =  c("distance","undistinguishable","environment","unexplained"), 
          radius = 0.9, main = paste0("Zeta = ", params$orders_msgdm[i])))
