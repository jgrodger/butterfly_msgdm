...
```{r}
corStructs <- c("corGaus(form = ~x+y)", "corExp(form = ~x+y)", "corLin(form = ~x+y)", "corRatio(form = ~x+y)", "corSpher(form = ~x+y)" )
...
my.gamm <- gamm(species.richness ~ s(mean.ann.temp) + s(log10.mean.ann.rain) + s(topographic.wetness) + s(ph) + s(total.n) + s(tree.density) + s(log10.pesticide.risk) + s(log10.humans),  data = site.by.env.xy.richness.distinct, method = "REML", correlation = corStructs[i], nugget = TRUE)
...
# gammGaus <- gamm(species.richness ~ s(mean.ann.temp) + s(log10.mean.ann.rain) + s(topographic.wetness) + s(ph) + s(total.n) + s(tree.density) + s(log10.pesticide.risk) + s(log10.humans),  data = site.by.env.xy.richness.distinct, method= "REML", correlation=corGaus(form = ~x+y))
# 
...
```{r}
aic_table <- AIC(gammGaus$lme, gammExp$lme, gammLin$lme, gammRatio$lme, gammSpher$lme)
aic_table
