...
# All species dataset is obtained by making a list of parts
...
# The other datsets are obtained using a function
...
#There are no sites with zero wider countryside species
...
datasets <- list(all_species, wc, hs, hs.no.zeros)
names(datasets) <- c("all_species", "wc", "hs", "hs_no_zeros")
```

#### for each dataset, prepare data as a dataframe for gamm input, remove transects that are duplicates for x and y coordinates (keep first one). This happens when sites are very close together and same coordinates are used
```{r}
...
all_species <- datasets$all_species
wc <- datasets$wc
hs <- datasets$hs
hs_no_zeros <- datasets$hs_no_zeros
```
