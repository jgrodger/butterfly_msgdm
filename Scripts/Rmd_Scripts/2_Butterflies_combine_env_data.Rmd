...
```{r}
species <- data.frame(row.names(site.by.species)) %>%
  cbind(rowSums(site.by.species))
names(species) <- c("site.no", "species.richness")

site.by.env <- merge(site.by.env, species, by = "site.no")
...
site.by.env <- select(site.by.env, -c(site.no, ngr.1km,  x, y, species.richness))
