...
#the numbers below are the number of grid cells with data

site.by.env <- site.by.temp %>%
  filter(ngr.1km %in% sites$ngr.1km ) %>%
...
site.by.site.name <- sites %>%
  select(site, ngr.1km)

site.by.env <- merge(site.by.env, site.by.site.name, by = "ngr.1km")

#this final number goes up as there are often multiple sites per grid cell
nrow(site.by.env)
...
```{r}
cor(site.by.env[, 4:11], method = "pearson")
```


```{r}
cor(site.by.env[, 4:11], method = "spearman")
...
site.by.env <- select(site.by.env, -c(ngr.1km, x, y))
```
...
names(data.2018) <- c("species", "sites", "site.by.species", "site.by.env", "site.by.xy")
