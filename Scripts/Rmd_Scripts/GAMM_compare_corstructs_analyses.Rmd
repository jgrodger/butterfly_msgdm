...
#### Habitat specialist, cubic regression splines, nb
```{r}
#checklist to modify after copy past
...
for (i in 1:length(corStructs)) {
my_gamm <- gamm(
  species.richness ~ s(mean.ann.temp, k = 6) + s(log10.mean.ann.rain, k = 6) + 
  s(topographic.wetness, k = 6) + s(ph, k = 4) + s(total.n, k = 6) + s(tree.density, k = 6) + 
  s(log10.pesticide.risk, k = 6) + s(log10.humans, k = 6), 
  method = "REML", 
  family = negbin(theta = 4.77), 
  correlation = corStructs[[i]],
  control=lmeCtlList , 
