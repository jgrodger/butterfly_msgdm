...
for (i in 1:length(datasets)){
  ...
  print(kable(performance, digits = 3,
             caption = paste0("Table SZ", i, ". Comparison of exponential versus power law fits to zeta decline for ", dataset_names[i])))
}
```
...
print(kable(t(zeta_div), digits = 2,
             caption = paste0("Table SZ", i + 4, ". Comparison of zeta diversity values for ", dataset_names[i])))
}
```
...
```{r}
print_rds(here("Output/Spatial/zeta_declines_decays/all_species/nestedness_anova_table.rds"), tab_no = "SZ9")
```
...
#### All Butterfly Species
```{r fig.cap = "Figure SZ12. Zeta diversity decay with distance for , zeta orders 1 to 100", out.width = "100%"}
include_graphics(here("./Output/Spatial/zeta_declines_decays/all_species/all_species_zeta_distance_decays.png"))
```

#### Wider Countryside Species
```{r fig.cap = "Figure SZ13. Zeta diversity decay with distance for , zeta orders 1 to 100", out.width = "100%"}
include_graphics(here("./Output/Spatial/zeta_declines_decays/wc/wc_zeta_distance_decays.png"))
```

#### Habitat Specialists including sites with no species
```{r fig.cap = "Figure SZ14. Zeta diversity decay with distance for , zeta orders 1 to 100", out.width = "100%"}
include_graphics(here("./Output/Spatial/zeta_declines_decays/hs/hs_zeta_distance_decays.png"))
```

#### Habitat Specialists excluding sites with no species
```{r fig.cap = "Figure SZ15. Zeta diversity decay with distance for , zeta orders 1 to 100", out.width = "100%"}
